...
export const capabilities:Record<Role,string[]>={
  CASE_ADMIN:[...basic,'EVIDENCE_UPLOAD','EVIDENCE_EXPORT','CUSTODY_TRANSFER','AI_QUERY','RECONSTRUCTION_CREATE','RECONSTRUCTION_FINALIZE','ATTEST','CASE_ADMIN'],
  INVESTIGATOR:[...basic,'EVIDENCE_UPLOAD','CUSTODY_TRANSFER','AI_QUERY','RECONSTRUCTION_CREATE'],
  FORENSIC_ANALYST:[...basic,'EVIDENCE_UPLOAD','AI_QUERY','RECONSTRUCTION_CREATE'],
  LEGAL_REVIEWER:[...basic,'AI_QUERY','EVIDENCE_EXPORT','RECONSTRUCTION_FINALIZE','ATTEST'],
  EXTERNAL_REVIEWER:[...basic],
  READ_ONLY:basic,
};
...
export function validateClaims(claims:any[],versions:Set<string>){
  for(const c of claims){
    if(!categories.includes(c.category))throw new Error('Invalid claim category');
    if(['VERIFIED_FACT','CORROBORATED_CLAIM','CONFLICT'].includes(c.category)&&!c.citations?.length)throw new Error('Grounded claim missing citation');
    for(const ref of c.citations||[])if(!versions.has(ref.versionId)||!ref.span)throw new Error('Invalid citation');
    if(c.category==='CORROBORATED_CLAIM'&&new Set(c.citations.map((r:any)=>r.versionId)).size<2)throw new Error('Corroboration requires distinct sources');
  }
  return claims;
}
